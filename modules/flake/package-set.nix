...
let
  overlays = [
    ...
    (final: _prev: {
      pi-coding-agent =
        builtins.scoped.commonInputs.llm-agents.packages.${final.stdenv.hostPlatform.system}.pi.override
          { useBun = false; };
    })
  ];
in
{
  ...
  perSystem =
    { system, ... }:
    {
      _module.args.pkgs = import builtins.scoped.commonInputs.nixpkgs {
        inherit system;
        inherit overlays;
      };
    };
