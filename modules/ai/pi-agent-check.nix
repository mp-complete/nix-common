...
{ config, ... }:
{
  perSystem =
    ...
    let
      host = builtins.scoped.commonInputs.nixpkgs.lib.nixosSystem {
        modules = [
          config.flake.modules.nixos.pi-agent
          builtins.scoped.commonInputs.sops-nix.nixosModules.sops
          (
            { lib, ... }:
            {
              ...
              # Exercise a repository-backed job without consumer secrets.
              piAgent.jobs.common-docs = {
                description = "pi-agent eval repository sample";
                repo.url = "https://github.com/mp-complete/nix-common.git";
                prompt = "Review the documentation and stop.";
                tools = [ "read" ];
              };
